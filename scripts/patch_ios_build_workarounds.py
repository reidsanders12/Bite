...
1. iOS deployment target 13.0 -> 15.0 (Podfile `platform :ios` line, and
   IPHONEOS_DEPLOYMENT_TARGET in all three Xcode build configs). Originally
   bumped to 14.0 only (the minimum the `health` plugin, flet-health,
   Profile -> Connect Health App, needs to `pod install` cleanly -- the flet
   0.28.3 stock template's default of 13.0 fails with "the plugin health
   requires a higher minimum iOS deployment version"), but App Store
   Connect's own upload validation separately rejected 14.0 with
   "MinimumOSVersion is too low ... should be 15.0 or later" -- that's an
   Apple-side floor on new submissions, not a specific plugin requirement,
   so 15.0 is now the target everywhere. There's no pyproject.toml hook for
   this in flet 0.28.3 (unlike Android's [tool.flet.android] min_sdk_version),
   so it has to be patched post-generation.
...
def patch_podfile() -> bool:
    ...
    changed = False
    for old_version in ("11.0", "12.0", "13.0", "14.0"):
